					    ...
					    try {
							...
							if(data.startsWith("G")){
								...
								if(con==null){
									...
									int i = stmt.executeUpdate("insert into booking_details(category, date_d, customer_no) values('GAS',now(),'1234567890')");
									if(i>0){
										System.out.println("Values are inserted....");
									//	doit(gpslocation);
									SendSMS sendSMS=new SendSMS();
									sendSMS.sendSMS("7010739604", "Gas BOOking for Customer:123456 required.");
									sendSMS.sendSMS("9789029498", "Gas Booked..");
									status=false;
									}else{
